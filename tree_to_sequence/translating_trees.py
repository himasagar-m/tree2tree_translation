...
def add_relation_for(tree):
    for child in tree.children:
        child.relation = tree.value
        add_relation_for(child)
def add_relation_lambda(tree):
    for child in tree.children:
        child.relation = tree.value
        add_relation_for(child)
        ...
        if not key in ignore_words:
            ...
            if isinstance(children, list):
                parentNode.children.extend(
                    map(lambda child: make_tree_javascript(child, long_base_case=long_base_case), children))
...
def make_tree_csharp(json, long_base_case=True):
    ...
    for key in json.keys():
        ...
        tag = "<" + key.upper() + ">"
        verbose_tokens = ["<CHARACTERLITERALEXPRESSION>", "<NUMERICLITERALEXPRESSION>", "<STRINGLITERALEXPRESSION>",
                          "<IDENTIFIERNAME>"]
...
def map_tree_edge(parent):
    '''
    node is encoded with node_value  and realtion to parent , for root node the relaton embedding is
    zero vector
    '''
    for child in parent.children:
        child.value = torch.cat((child.value, parent.value), 0)
        map_tree_edge(child)
...
def category_to_child_coffee(num_vars, num_ints, category):
    ...
    coffee_grammar = {
        ...
        CoffeeGrammar.STATEMENT: [x + n for x in
                                  [Coffee.SHORTSTATEMENTCS, Coffee.IFTHENELSE, Coffee.IFELSE, Coffee.WHILE, Coffee.IF]],
    }
